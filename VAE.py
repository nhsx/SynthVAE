...
class Encoder(nn.Module):
  ...
  def __init__(self, input_dim, latent_dim, hidden_dim=32, activation=nn.Tanh,
               device="cpu"):
    super().__init__()
    if device == "gpu":
...
class Decoder(nn.Module):
  ...
  def __init__(self, latent_dim, num_continuous, num_categories=[0], hidden_dim=32, activation=nn.Tanh,
               device="cpu"):
    super().__init__()
...
class VAE(nn.Module):
  ...
  def generate(self, N):
    ...
    for v in range(len(self.num_categories)):
      x_gen_[:,i:(i+self.num_categories[v])] = torch.distributions.one_hot_categorical.OneHotCategorical(logits=x_gen[:, i:(i+self.num_categories[v])]).sample()
      i  = i + self.num_categories[v]
    x_gen_[:, -self.num_continuous:] = x_gen[:, -self.num_continuous:] + torch.exp(self.noiser(x_gen[:, -self.num_continuous:])) * torch.randn_like(x_gen[:, -self.num_continuous:])
    return x_gen_
  ...
  def loss(self, X):
    ...
    if sum(self.num_categories) != 0:
      i = 0
      for v in range(len(self.num_categories)):
        categoric_loglik += -torch.nn.functional.cross_entropy(x_recon[:,i:(i+self.num_categories[v])], torch.max(X[:,i:(i+self.num_categories[v])],1)[1]).sum()
        i  = i + self.decoder.num_categories[v]

    gauss_loglik = 0
    if self.decoder.num_continuous != 0:
      gauss_loglik = Normal(loc=x_recon[:,-self.num_continuous:], scale=torch.exp(self.noiser(x_recon[:,-self.num_continuous:]))).log_prob(X[:,-self.num_continuous:]).sum()
  ...
  def diff_priv_train(self, x_dataloader, n_epochs, C=1e16, noise_scale=None,
                      target_eps=1, target_delta=1e-5, logging_freq=1,
                      sample_rate=0.1):
  ...
  def get_privacy_spent(self, delta):
    if hasattr(self,"privacy_engine"):
      ...
      print("This VAE object does not a privacy_engine attribute. Run diff_priv_train to create one.")
